...
Since Var($$t_{1i}$$) $$= \textbf{a_{2}'}\textbf{S}\textbf{a_{2}}$$ its maximum is the largest eigenvalue $\lamba_{1}$, so $$\textbf{a}_{1}$$ is attained by choosing the correspective eigenvalue of $textbf{S}$.
